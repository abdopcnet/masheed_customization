...
def execute(filters=None):
    ...
    group_wise_columns = frappe._dict({
        "invoice": ["created_by", "parent", "customer", "customer_group", "posting_date", "item_code", "item_name", "item_group", "brand", "description",
                    "warehouse", "qty", "base_rate", "buying_rate", "base_amount",
                    "buying_amount", "gross_profit", "gross_profit_percent", "gross_profit_based_vr", "project"],
        "item_code": ["item_code", "item_name", "brand", "description", "qty", "base_rate",
                      "buying_rate", "base_amount", "buying_amount", "gross_profit", "gross_profit_percent", "gross_profit_based_vr"],
        "warehouse": ["warehouse", "qty", "base_rate", "buying_rate", "base_amount", "buying_amount",
                      "gross_profit", "gross_profit_percent", "gross_profit_based_vr"],
        ...
        "sales_person": ["sales_person", "allocated_amount", "qty", "base_rate", "buying_rate", "base_amount", "buying_amount",
                         "gross_profit", "gross_profit_percent", "gross_profit_based_vr"],
        "project": ["project", "base_amount", "buying_amount", "gross_profit", "gross_profit_percent", "gross_profit_based_vr"],
        "territory": ["territory", "base_amount", "buying_amount", "gross_profit", "gross_profit_percent", "gross_profit_based_vr"]
    })
...
def get_columns(group_wise_columns, filters):
    ...
    column_map = frappe._dict({
        "created_by": _("Created By") + ":Link/User:120",
        ...
        "gross_profit_based_vr": _("Gross Profit Based on VR %") + ":Percent:100",
...
class GrossProfitGenerator(object):
    ...
    def process(self):
        ...
        for row in self.si_list:
            ...
            if row.qty:
                row.buying_rate = flt(
                    row.buying_amount / row.qty, self.float_precision)
                row.base_rate = flt(
                    row.base_amount / row.qty, self.float_precision)
            else:
            ...
            if row.buying_rate:
                row.gross_profit_based_vr = flt(
                    ((row.base_amount / row.qty / row.buying_rate) - 1) * 100.0, self.currency_precision)
    ...
    def set_average_rate(self, new_row):
        ...
        new_row.buying_rate = flt(
            new_row.buying_amount / new_row.qty, self.float_precision) if new_row.qty else 0
        new_row.base_rate = flt(
            new_row.base_amount / new_row.qty, self.float_precision) if new_row.qty else 0
    ...
    def load_invoice_items(self):
        ...
        if self.filters.get("created_by"):
            conditions += " and `tabSales Invoice`.owner = %(created_by)s"
        ...
        self.si_list = frappe.db.sql("""
			select
				...
				`tabSales Invoice`.owner as created_by
				{sales_person_cols}
			from
